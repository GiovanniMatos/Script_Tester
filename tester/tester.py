...
def compare_html_css_js(html_file, css_file, js_file):
    ...
    input_fields = html_soup.find_all("input", type="text")
    
    if input_fields == []:
        print("Formulários não encontrados \n")
    else:
        # Validar campos de entrada de texto
        for field in input_fields:
            if field.get("required") and field.get("value") == "":
                print("Campo obrigatório não preenchido:", field)
    print()
